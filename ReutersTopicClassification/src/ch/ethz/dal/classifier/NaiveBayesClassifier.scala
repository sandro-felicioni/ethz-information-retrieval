...
class NaiveBayesClassifier(datasetPath: String, maxDocuments: Int, alpha: Int, removeStopwords: Boolean, useStemming: Boolean) {
  ...
  private def getCleanTokens(tokens: List[String]) : List[String] = {
    ...
    //return tokens.map(token => token.toLowerCase()).filter(token => token.length() > 2 && !stopwords.contains(token))
    return cleanTokens
  }
