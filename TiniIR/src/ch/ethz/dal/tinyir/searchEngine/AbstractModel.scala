...
abstract class AbstractModel {
  ...
  private def getCleanTokens(doc: XMLDocument): List[String] = {
    return Tokenizer.toLowerCase(doc.tokens).filter(token => !stopwords.contains(token))
  }
  ...
  private def createFrequencyTriples(stream: Stream[XMLDocument]): List[(String, String, Double)] = {
    return stream.flatMap(doc => getCleanTokens(doc).groupBy(identity)
