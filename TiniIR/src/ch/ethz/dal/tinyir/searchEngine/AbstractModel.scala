...
abstract class AbstractModel(tipster: TipsterStream, numDocumentsToParse: Int = -1) {
  ...
  /** return the stream to parse the documents */
  protected def getStream() : Stream[XMLDocument] = {
    return if (numDocumentsToParse < 0) tipster.stream else tipster.stream.take(numDocumentsToParse)
  }
   ...
   */
  def computeScore(queries: List[(String, Int)], maxDocuments: Int): List[(Int, List[(String, Double)])] = {

    // initialize results
    var results = new mutable.HashMap[Int, mutable.PriorityQueue[(String, Double)]]()
